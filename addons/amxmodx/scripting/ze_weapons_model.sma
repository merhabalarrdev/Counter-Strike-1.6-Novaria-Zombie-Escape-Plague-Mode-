...
new new_sounds[MAX_SOUNDS][64]
new old_sounds[MAX_SOUNDS][64]
new sounds_team[MAX_SOUNDS]
new soundsnum

new new_p_models[MAX_p_MODELS][64]
new old_p_models[MAX_p_MODELS][64]
new p_models_team[MAX_p_MODELS]
new p_modelsnum

new new_v_models[MAX_v_MODELS][64]
new old_v_models[MAX_v_MODELS][64]
new v_models_team[MAX_p_MODELS]
new v_modelsnum

new new_w_models[MAX_w_MODELS][64]
new old_w_models[MAX_w_MODELS][64]
...
public load_models(configfile[])
{
	if(file_exists(configfile))
	{
		new read[128], left[64], right[64], right2[32], trash, team
		for(new i=0;i<file_size(configfile,1);i++)
		{
			read_file(configfile,i,read,127,trash)
			if(containi(read,";")!=0 && containi(read," ")!=-1)
			{
				argbreak(read,left,63,right,63)
				team=0
				if(containi(right," ")!=-1)
				{
					argbreak(right,right,63,right2,31)
				...
				if(file_exists(right))
				{
					if(containi(right,".mdl")==strlen(right)-4)
					{
						...
						{
							format(new_p_models[p_modelsnum],63,right)
							format(old_p_models[p_modelsnum],63,left)
							p_models_team[p_modelsnum]=team
							p_modelsnum++
						}
						else if(containi(left,"models/v_")==0)
						{
							format(new_v_models[v_modelsnum],63,right)
							format(old_v_models[v_modelsnum],63,left)
							v_models_team[v_modelsnum]=team
							v_modelsnum++
						}
						else if(containi(left,"models/w_")==0)
						{
							format(new_w_models[w_modelsnum],63,right)
							format(old_w_models[w_modelsnum],63,left)
					...
					{
						replace(right,63,"sound/","")
						replace(left,63,"sound/","")
						...
						{
							format(new_sounds[soundsnum],63,right)
							format(old_sounds[soundsnum],63,left)
