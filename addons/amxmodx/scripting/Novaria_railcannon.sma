...
new const MuzzleFlash[] = "sprites/zea_mod/muzzleflash19.spr"
...
const Float:damage_mode1 = 35.0
const Float:damage_mode2 = 70.0
const Float:damage_mode3 = 100.0

/////////////////////////////////////////////////////////////////
new RAILCANNON_V_MODEL[64] = "models/zea_mod/Weapons/railcannon_umbrella.mdl"
new RAILCANNON_P_MODEL[64] = "models/zea_mod/Weapons/p_railcannon.mdl"
new RAILCANNON_W_MODEL[64] = "models/zea_mod/Weapons/w_railcannon.mdl"
...
public plugin_init()
{
	register_plugin("Rail Cannon", "1.5", "m4m3ts/S3xTy Edit")
	...
	register_clcmd("GETrail", "give_railcannon", ADMIN_RCON)
	
	g_rail = zp_register_extra_item("Rail Cannon", 20, ZP_TEAM_HUMAN)
...
public plugin_precache()
{
	...
	precache_sound("weapons/railcanon_chage1.wav")
	precache_sound("weapons/railcanon_chage2.wav")
	precache_sound("weapons/railcanon_chage3.wav")
	...
	precache_generic("sprites/zea_railcannon.txt")
	
	precache_generic("sprites/zea_mod/640hud112.spr")
	precache_generic("sprites/zea_mod/640hud13.spr")
		
	register_clcmd("zea_railcannon", "weapon_hook")	
...
{
	if(itemid == g_rail) give_railcannon(id)
}
...
public fw_TraceAttack(iEnt, iAttacker, Float:flDamage, Float:fDir[3], ptr, iDamageType)
{
	...
	if(railcannon_mode[iAttacker] == 3)
	{
		...
		if(zp_get_user_zombie(iEnt) && g_KnockBack == 1) knockback(iEnt, myOrigin, 42.0, 2)
	}
...
public fw_CmdStart(id, uc_handle, seed) 
{
	...
	else if(CurButton & IN_ATTACK2)
	{
		if(!zz[id] && get_pdata_float(id, 83, 4) <= 0.0 && g_clip_ammo[id] >= 1)
		{
			...
			emit_sound(id, CHAN_WEAPON, "weapons/railcanon_chage1.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
			set_task(0.4, "charge_idle2", id)
			zz[id] = 1
			udah[id] = 0
...
public charge_idle2(id)
{
	...
	if(zz[id])
	{
		if(g_clip_ammo[id] >= 1)
		{
			...
			set_weapons_timeidle(id, CSW_M3, 200.0)
			set_player_nextattackx(id, 200.0)
			emit_sound(id, CHAN_WEAPON, "weapons/railcanon_chage2.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
			set_task(0.4, "charge_idle3", id)
...
public charge_idle3(id)
{
	...
	if(zz[id])
	{
		if(g_clip_ammo[id] >= 1)
		{
			...
			emit_sound(id, CHAN_WEAPON, "weapons/railcanon_chage3.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
