...
#define PLUGIN "[ZP] Extra: Cart Blue VIP"
#define VERSION "1.1"
...
public plugin_init()
{
	...
	g_iVipItemId = novaria_vip_extra_items("Cart Blue", "1", "100");
}
...
public HamHook_Item_AttachToPlayer(const iItem, const iPlayer)
{
	...
	if (IsCustomItem(iItem))
	{
		...
		SendWeaponListUpdate(iPlayer, WEAPON_NAME_C);
		SendWeaponListUpdate(iPlayer, WEAPON_NAME_S, true);
		
		static msgWeapPickup;
		if (msgWeapPickup || (msgWeapPickup = get_user_msgid("WeapPickup")))
		{
			MESSAGE_BEGIN(MSG_ONE, msgWeapPickup, {0.0, 0.0, 0.0}, iPlayer);
...
{
	static iItem;
	if (CheckItem2(iPlayer, iItem) && get_pdata_int(iItem, m_iId, extra_offset_weapon) == get_msg_arg_int(2))
	{
...
Weapon_ModeSwitch(const iPlayer, const iNewMode)
{
	...
	{
		Weapon_OnModeSwitch(iItem, iPlayer, get_pdata_int(iItem, m_iClip, extra_offset_weapon), GetAmmoInventory(iPlayer, PrimaryAmmoIndex(iItem)), iNewMode);
	}
}

SendWeaponListUpdate(const iPlayer, const szWeaponName[32], const bool: bDummy = false)
{
	MESSAGE_BEGIN(MSG_ONE, MSG_WEAPONLIST, {0.0, 0.0, 0.0}, iPlayer);
	WRITE_STRING(szWeaponName);
	WRITE_BYTE(g_aWeaponListData[0]);
	WRITE_BYTE(g_aWeaponListData[1]);
	WRITE_BYTE(g_aWeaponListData[2]);
	WRITE_BYTE(g_aWeaponListData[3]);
	WRITE_BYTE(g_aWeaponListData[4]);
	...
	{
		WRITE_BYTE(g_aWeaponListData[5]);
		WRITE_BYTE(g_aWeaponListData[6]);
	}
	
	WRITE_BYTE(g_aWeaponListData[7]);
	MESSAGE_END();
}
...
public client_putinserver(id) { BitSet(g_bitIsConnected, id); }
public client_disconnected(id) { BitClear(g_bitIsConnected, id); }

bool: CheckItem(const iItem, &iPlayer)
{
	if (!IsValidPev(iItem) || !IsCustomItem(iItem)) return false;
	iPlayer = get_pdata_cbase(iItem, m_pPlayer, extra_offset_weapon);
	if (!IsValidPev(iPlayer) || !BitCheck(g_bitIsConnected, iPlayer)) return false;
	return true;
}

bool: CheckItem2(const iPlayer, &iItem)
{
	if (!BitCheck(g_bitIsConnected, iPlayer) || !IsValidPev(iPlayer)) return false;
...
Weapon_Create(const Float: vecOrigin[3] = {0.0, 0.0, 0.0}, const Float: vecAngles[3] = {0.0, 0.0, 0.0})
{
	...
	if (!IsValidPev(iWeapon)) return FM_NULLENT;
...
{
	if (!IsValidPev(iPlayer)) return FM_NULLENT;
...
{
	static i, msgBrass;
	static Float: vecUp[3], Float: vecRight[3], Float: vecForward[3]; 
	static Float: vecAngle[3], Float: vecOrigin[3], Float: vecViewOfs[3], Float: vecVelocity[3];
...
Weapon_KickBack(const iItem, const iPlayer, Float: upBase, Float: lateralBase, const Float: upMod, const Float: lateralMod, Float: upMax, Float: lateralMax, const directionChange)
{
	...
	if (upMax >= vecPunchangle[0]) vecPunchangle[0] = upMax;
	
	if ((iDirection = get_pdata_int(iItem, m_iDirection, extra_offset_weapon)))
	{
		vecPunchangle[1] += lateralBase;
		if (lateralMax < vecPunchangle[1]) vecPunchangle[1] = lateralMax;
	}
	else
	{
		lateralMax *= -1.0;
...
{
	static iHit, iMessage, iDecalIndex;
	static Float: flFraction, Float: vecEndPos[3];
	
	iHit = INSTANCE(get_tr2(iTrace, TR_pHit));
	if (iHit && !IsValidPev(iHit) || (pev(iHit, pev_flags) & FL_KILLME)) return;
	if (pev(iHit, pev_solid) != SOLID_BSP && pev(iHit, pev_movetype) != MOVETYPE_PUSHSTEP) return;
	
	iDecalIndex = ExecuteHamB(Ham_DamageDecal, iHit, 0);
	if (iDecalIndex < 0 || iDecalIndex >= ArraySize(g_hDecals)) return;
	
	iDecalIndex = ArrayGetCell(g_hDecals, iDecalIndex);
	get_tr2(iTrace, TR_flFraction, flFraction);
	get_tr2(iTrace, TR_vecEndPos, vecEndPos);
	
	if (iDecalIndex < 0 || flFraction >= 1.0) return;
	...
	else
	{
		...
		if (iHit != 0)
		{
			if (iDecalIndex > 255) { iMessage = TE_DECALHIGH; iDecalIndex -= 256; }
		}
		else
		{
			iMessage = TE_WORLDDECAL;
			if (iDecalIndex > 255) { iMessage = TE_WORLDDECALHIGH; iDecalIndex -= 256; }
		}
	}
	...
	if (bIsGunshot) { WRITE_SHORT(iHit); WRITE_BYTE(iDecalIndex); }
	else { WRITE_BYTE(iDecalIndex); if (iHit) WRITE_SHORT(iHit); }
	MESSAGE_END();
}
	...
	if ((iFile = fopen(szModelPath, "rt")))
	{
		...
		for (new k, i = 0; i < iNumSeq; i++)
		{
			...
			for (k = 0; k < iNumEvents; k++)
			{
				...
				if (strlen(szSoundPath)) { strtolower(szSoundPath); PRECACHE_SOUND(szSoundPath); }
			}
		}
	}
	fclose(iFile);
...
PrimaryAmmoIndex(const iItem) { return get_pdata_int(iItem, m_iPrimaryAmmoType, extra_offset_weapon); }

GetAmmoInventory(const iPlayer, const iAmmoIndex)
{
	if (iAmmoIndex == -1) return -1;
