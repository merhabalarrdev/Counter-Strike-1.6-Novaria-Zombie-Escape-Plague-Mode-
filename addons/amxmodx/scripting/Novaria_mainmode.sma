...
#pragma unused g_iNovariaTemaAktif

LoadNovariaTemaLocal()
{
	new hVault = nvault_open("rezpvault11");
	if (hVault == -1)
		return;

	new szData[16];
	new iTimestamp;
	if (nvault_lookup(hVault, "novaria_tema_aktif", szData, charsmax(szData), iTimestamp))
	{
		new iTema = str_to_num(szData);
		if (iTema >= 0 && iTema < sizeof(szNovariaTema))
		{
...
// KALDIRILDI: bu eklentinin kendi rol/tag sistemi (szFlagConnectControll,
// IP_IDszString, szChatTagFormat) mainmenu'deki role/dil sistemiyle cakisiyordu
// (say/say_team'i yakalayip PLUGIN_HANDLED donduruyordu). Rol/tag artik
// sadece mainmenu (Novaria_zemainmenu.sma) tarafindan yonetiliyor.
native coin_get_user(IP_IDs);
native get_user_level(IP_IDs);
...
#define MENU_PAGE_BUY1 g_menu_data[id][8]
...
// Primary and Secondary Weapon Names artik zombie_plague.txt icinde WPNNAME_<csw_id>
// anahtarlariyla dilden okunuyor (bkz. get_localized_weapon_name), bu dizi kaldirildi.
...
// Silah Al kategori tercihi (0=Tumu, 1=Hafif, 2=Agir) - oturum boyunca hatirlanir
new g_wpnCategory[33]

// Agir silah sayilan CSW id'leri (Sniper / Makineli / Pompali)
is_heavy_weapon(weaponid)
{
	switch (weaponid)
	{
		case CSW_AWP, CSW_SCOUT, CSW_G3SG1, CSW_SG550, CSW_M249, CSW_XM1014, CSW_M3:
			return true;
	}
	return false;
}

// Silahin dilden (zombie_plague.txt icindeki WPNNAME_<csw_id>) okunan ismini dondurur
stock get_localized_weapon_name(id, weaponid, output[], len)
{
	static key[16]
	formatex(key, charsmax(key), "WPNNAME_%d", weaponid)
	formatex(output, len, "%L", id, key)
}
...
new g_menu_data[33][9] // data for some menu handlers
...
Array:g_modelindex_admin_zombie, Array:g_modelindex_admin_human, 
...
Array:sound_nemesis_ismp3, Array:sound_assassin_ismp3, Array:sound_survivor_ismp3, Array:sound_sniper_ismp3,
Array:sound_swarm_ismp3, Array:sound_multi_ismp3, Array:sound_plague_ismp3,
Array:sound_armageddon_ismp3, Array:sound_apocalypse_ismp3, Array:sound_nightmare_ismp3,
...
public plugin_natives()
{
	...
	register_native("zp_override_user_model", "native_override_user_model", 1)
	register_native("zp_open_zclass_menu", "native_open_zclass_menu", 1)
	...
	register_native("zp_get_extra_items_count", "native_get_extra_items_count", 0)
	register_native("zp_get_extra_item_info", "native_get_extra_item_info", 0)
}

// Native: zp_get_extra_items_count
// Su an kayitli olan toplam extra esya sayisini (hardcoded 4 taneyle beraber,
// baska pluginlerin zp_register_extra_item ile ekledikleri de dahil) dondurur.
public native_get_extra_items_count(iPlugin, iParams)
{
	return g_extraitem_i;
}

// Native: zp_get_extra_item_info
// zp_get_extra_item_info(itemid, name[], namelen, &cost, &team)
// Verilen itemid'nin ismini, fiyatini ve takim bitmask'ini (ZP_TEAM_*) doldurur.
// Basarili ise 1, gecersiz itemid ise 0 doner.
public native_get_extra_item_info(iPlugin, iParams)
{
	...
	set_param_byref(4, ArrayGetCell(g_extraitem_cost, itemid))
	set_param_byref(5, ArrayGetCell(g_extraitem_team, itemid))

	return 1;
}

public native_open_zclass_menu(id)
{
...
public plugin_precache()
{
	...
	register_plugin("Novaria Zombie Escape", PLUGIN_VERSION, "MeRcyLeZZ/Merhabalarr")
	...
	sound_nemesis_ismp3 = ArrayCreate(1, 1)
	sound_assassin = ArrayCreate(64, 1)
	sound_assassin_ismp3 = ArrayCreate(1, 1)
	sound_survivor = ArrayCreate(64, 1)
	sound_survivor_ismp3 = ArrayCreate(1, 1)
	sound_sniper = ArrayCreate(64, 1)
	sound_sniper_ismp3 = ArrayCreate(1, 1)
	sound_swarm = ArrayCreate(64, 1)
	sound_swarm_ismp3 = ArrayCreate(1, 1)
	sound_multi = ArrayCreate(64, 1)
	sound_multi_ismp3 = ArrayCreate(1, 1)
	sound_plague = ArrayCreate(64, 1)
	sound_plague_ismp3 = ArrayCreate(1, 1)
	sound_armageddon = ArrayCreate(64, 1)
	sound_armageddon_ismp3 = ArrayCreate(1, 1)
	sound_apocalypse = ArrayCreate(64, 1)
	sound_apocalypse_ismp3 = ArrayCreate(1, 1)
	sound_nightmare = ArrayCreate(64, 1)
	sound_nightmare_ismp3 = ArrayCreate(1, 1)
	...
	// [FIX #2] Bu satir yorum satiriydi. EXTRA_NVISION enum'da 0 olarak ayrilmisti
	// ama kayit edilmeyince Antidote gercekte id=0, Madness id=1, InfBomb id=2 olarak
	// kaydoluyordu - halbuki buy_extra_item() switch'i hala enum sabitlerini (1,2,3)
	// kullaniyordu. Sonuc: Antidote sectiginde id=0 -> case EXTRA_NVISION calisiyordu
	// (hicbir sey olmuyordu), InfBomb sectiginde id=2 -> case EXTRA_MADNESS calisiyordu.
	// Kaydi geri acmak, runtime item ID'lerini enum sabitleriyle tekrar hizaliyor.
	native_register_extra_item2("NightVision", g_extra_costs2[EXTRA_NVISION], ZP_TEAM_HUMAN)
	native_register_extra_item2("\r[\yT-Virus Antidote\r]", g_extra_costs2[EXTRA_ANTIDOTE], ZP_TEAM_ZOMBIE)
	native_register_extra_item2("\r[\yZombie Çılgınlığı\r]", g_extra_costs2[EXTRA_MADNESS], ZP_TEAM_ZOMBIE)
	native_register_extra_item2("\r[\yEnfeksiyon Bombası\r]", g_extra_costs2[EXTRA_INFBOMB], ZP_TEAM_ZOMBIE)
	...
	for (i = 0; i < ArraySize(sound_nemesis); i++)
	{
		...
		if (ArrayGetCell(sound_nemesis_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
	...
	for (i = 0; i < ArraySize(sound_assassin); i++)
	{
		...
		if (ArrayGetCell(sound_assassin_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
	...
	for (i = 0; i < ArraySize(sound_survivor); i++)
	{
		...
		if (ArrayGetCell(sound_survivor_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
	...
	for (i = 0; i < ArraySize(sound_sniper); i++)
	{
		...
		if (ArrayGetCell(sound_sniper_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
	...
	for (i = 0; i < ArraySize(sound_swarm); i++)
	{
		...
		if (ArrayGetCell(sound_swarm_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
	...
	for (i = 0; i < ArraySize(sound_multi); i++)
	{
		...
		if (ArrayGetCell(sound_multi_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
	...
	for (i = 0; i < ArraySize(sound_plague); i++)
	{
		...
		if (ArrayGetCell(sound_plague_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
	...
	for (i = 0; i < ArraySize(sound_armageddon); i++)
	{
		...
		if (ArrayGetCell(sound_armageddon_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
	...
	for (i = 0; i < ArraySize(sound_apocalypse); i++)
	{
		...
		if (ArrayGetCell(sound_apocalypse_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
	...
	for (i = 0; i < ArraySize(sound_nightmare); i++)
	{
		...
		if (ArrayGetCell(sound_nightmare_ismp3, i))
		{
			format(buffer, charsmax(buffer), "sound/%s", buffer)
			engfunc(EngFunc_PrecacheGeneric, buffer)
		}
...
public plugin_init()
{
	...
	// rAs tag/tema sistemini zemainmenu ile ayni vault'tan senkronla
	LoadNovariaTemaLocal()
	...
	// Client commands
	register_clcmd("say zpmenu", "sRegistered")
	register_clcmd("say /zpmenu", "sRegistered")
	...
	register_clcmd("say /agugumatata", "zpmainmenus");
	// KALDIRILDI: register_clcmd("say"/"say_team", "sHookChatTag") — rol/tag artik mainmenu'de.
	
	// Menus
	register_menu("Game Menu", KEYSMENU, "menu_game")
	// "Buy Menu 1" artik menu_create tabanli, tema sistemine bagli, sayfa hatirlayan
	// bir menu - register_menu(raw key callback) ile CAKISMASIN diye buradan kaldirildi.
	...
	cvar_leapzombiesforce = register_cvar("zp_leap_zombies_force", "1000")
	...
	cvar_survinfammo = register_cvar("zp_surv_unlimited_ammo", "0")
	...
	// Format mod name
	formatex(g_modname, charsmax(g_modname), "Novaria Zombie Escape %s", PLUGIN_VERSION)
...
public fw_PlayerSpawn_Post(id)
{
	...
	if (get_pcvar_num(cvar_buycustom))
		set_task(0.2, "show_buymenu_start", id+TASK_SPAWN)
...
{
	// KALDIRILDI: sYetkiTanimla(id) — rol/tag artik mainmenu'de.
	// Plugin disabled?
	if (!g_pluginenabled) return;
...
// KALDIRILDI: sYetkiTanimla() ve sHookChatTag() — bu eklentinin kendi rol/tag
// sistemi, say/say_team'i yakalayip PLUGIN_HANDLED donduruyor ve mainmenu'nun
// kendi [Rutbe] [Rol] Isim: mesaj chat sistemiyle cakisiyordu. Artik chat tag
// tamamen Novaria_zemainmenu.sma tarafindan yonetiliyor.
// Client leaving
...
/*================================================================================
 [Menus]
=================================================================================*/
public sRegistered(const IP_IDs) {
	zpmainmenus(IP_IDs);
	return PLUGIN_HANDLED;
}
public zpmainmenus(const IP_IDs) {
	// Novaria: Ana menunun tasarimi ve icerigi artik zp_mainmenuv13.sma icinde
	// (@sGetPlayerNovariaAnaMenuCreate) yonetiliyor. Eski menu devre disi, cagriyi oraya yonlendiriyoruz.
	client_cmd(IP_IDs, "novariamenu_ac")
	return PLUGIN_HANDLED;
}
public zpmain_dvm(const id,const iMenu, const iItem) {
	...
	if(iL_Key == 9){ menu_destroy(iMenu);return PLUGIN_HANDLED; }
	switch(iL_Key) {
		case 1: { 
		if (get_pcvar_num(cvar_buycustom))
			{
				...
				// Show menu if player hasn't yet bought anything, aksi halde zaten aldigini bildir
				if (g_canbuy[id])
					show_menu_buycat(id)
				else
					zp_colored_print(id, "^x04[Zp]^x01 %L", id, "MENU_BUY1_ALREADY")
			}

			else
				zp_colored_print(id, "^x04[AvP]^x01 %L", id, "CMD_NOT")
        }
		case 2: { 
        // Extra items enabled?
			if (get_pcvar_num(cvar_extraitems))
			{
				...
				zp_colored_print(id, "^x04[AvP]^x01 %L", id, "CMD_NOT_EXTRAS")
        }	
		case 3: { 
		// Zombie classes enabled?
			if (get_pcvar_num(cvar_zclasses))
				...
				zp_colored_print(id, "^x04[AvP] ^x01 %L", id, "CMD_NOT_ZCLASSES")
        } 
        case 50: { client_cmd(id, "say /insanclass"); }
        case 4: { client_cmd(id, "say /CoinMenu"); } 
        case 5: { client_cmd(id, "say /KnifeMenu"); } 
        case 6: { client_cmd(id, "say /UserMenu"); } 
        ...
        // Check if player has the required access
			if (get_user_flags(id) & g_access_flag[ACCESS_ADMIN_MENU])
				...
				zp_colored_print(id, "^x04[AvP]^x01 %L", id, "CMD_NOT_ACCESS")
        } 
	case 11 : {  return PLUGIN_HANDLED; }
	}
	menu_destroy(iMenu);return PLUGIN_HANDLED;
}
stock szMenuEnd(sUserIndex,sMenuID,sSmallMenu = 1,sExit = 1,sPage = 0){
...
// Silah Kategorisi Secim Menusu (Hafif / Agir / Tumu)
show_menu_buycat(id)
{
	// Player dead?
	if (!g_isalive[id])
	...
	static menuid, menu[128], buffer[4]

	// Title - tema sistemine bagli (szTag)
	formatex(menu, charsmax(menu), "\y%s \w%L\r", szTag[MenuTag], id, "MENU_BUYCAT_TITLE")
	menuid = menu_create(menu, "menu_buycat")

	// Tumu
	formatex(menu, charsmax(menu), "\d%s \w%L %s", szTag[KisaTag], id, "MENU_BUYCAT_ALL", (g_wpnCategory[id] == 0) ? "\y*" : "")
	buffer[0] = 0
	menu_additem(menuid, menu, buffer)

	// Hafif Silahlar
	formatex(menu, charsmax(menu), "\d%s \w%L %s", szTag[KisaTag], id, "MENU_BUYCAT_LIGHT", (g_wpnCategory[id] == 1) ? "\y*" : "")
	buffer[0] = 1
	menu_additem(menuid, menu, buffer)

	// Agir Silahlar
	formatex(menu, charsmax(menu), "\d%s \w%L %s", szTag[KisaTag], id, "MENU_BUYCAT_HEAVY", (g_wpnCategory[id] == 2) ? "\y*" : "")
	buffer[0] = 2
	menu_additem(menuid, menu, buffer)

	// Back - Next - Exit (tema tagimizla)
	formatex(menu, charsmax(menu), "\d%s \w%L", szTag[KisaTag], id, "MENU_BACK")
	menu_setprop(menuid, MPROP_BACKNAME, menu)

	formatex(menu, charsmax(menu), "\d%s \w%L", szTag[KisaTag], id, "MENU_NEXT")
	menu_setprop(menuid, MPROP_NEXTNAME, menu)

	formatex(menu, charsmax(menu), "\d%s \w%L", szTag[KisaTag], id, "MENU_EXIT")
...
// Silah Kategorisi Secim Menusu Handler
public menu_buycat(id, menuid, item)
{
	// Player disconnected?
	if (!is_user_connected(id))
	{
	...
	// Secilen kategoriyi al
	static buffer[4], dummy, selection
	menu_item_getinfo(menuid, item, dummy, buffer, charsmax(buffer), _, _, dummy)
	selection = buffer[0]

	menu_destroy(menuid)

	// Tercihi kaydet (oturum boyunca hatirlanir)
	g_wpnCategory[id] = selection

	// Secilen kategoriye gore silah listesini ac
	show_menu_buy1(id)

	return PLUGIN_HANDLED
}

// Round basi Silah Al akisi (spawn task) - otomatik/bot/random durumlarini
// ayikladiktan sonra ONCE kategori (Tumu/Hafif/Agir) menusunu acar
public show_buymenu_start(taskid)
{
	// Get player's id
	static id
	...
	// Automatic selection enabled -> secimi hatirla, menu hic acilmasin
	if (WPN_AUTO_ON)
	{
	...
	// Manuel aliyor -> once kategori sec
	show_menu_buycat(id)
}
...
public show_menu_buy1(taskid)
{
	...
	static menuid, menu[128], weap, buffer[4], wpnname[64]

	// Title - tema sistemine bagli (szTag)
	formatex(menu, charsmax(menu), "\y%s \w%L\r", szTag[MenuTag], id, "MENU_BUY1_TITLE")
        menuid = menu_create(menu, "menu_buy1")

	// Weapon List (item data = silahin g_primary_items icindeki index'i)
	// Sadece secili kategoriye (Tumu/Hafif/Agir) uyan silahlar listelenir
	for (weap = 0; weap < WPN_MAXIDS; weap++)
	{
		if (g_wpnCategory[id] == 1 && is_heavy_weapon(ArrayGetCell(g_primary_weaponids, weap)))
			continue;
		if (g_wpnCategory[id] == 2 && !is_heavy_weapon(ArrayGetCell(g_primary_weaponids, weap)))
			continue;

		get_localized_weapon_name(id, ArrayGetCell(g_primary_weaponids, weap), wpnname, charsmax(wpnname))
		formatex(menu, charsmax(menu), "\d%s \w%s", szTag[KisaTag], wpnname)
		buffer[0] = weap
		buffer[1] = 0
		menu_additem(menuid, menu, buffer)
	}
	...
	// Kategori Degistir
	formatex(menu, charsmax(menu), "\d%s \w%L", szTag[KisaTag], id, "MENU_BUY1_CHANGECAT")
	buffer[0] = -2
	buffer[1] = 0
	menu_additem(menuid, menu, buffer)

	// Auto Select
	formatex(menu, charsmax(menu), "\d%s \w%L \y[%L]", szTag[KisaTag], id, "MENU_AUTOSELECT", id, (WPN_AUTO_ON) ? "MOTD_ENABLED" : "MOTD_DISABLED")
	buffer[0] = -1
	buffer[1] = 0
	menu_additem(menuid, menu, buffer)
	...
	// Hatirlanan sayfadan kaldigi yerden ac (sayfa sayisini asarsa sinirla)
	MENU_PAGE_BUY1 = min(MENU_PAGE_BUY1, menu_pages(menuid)-1)

	// Fix for AMXX custom menus
	if (pev_valid(id) == PDATA_SAFE)
		set_pdata_int(id, OFFSET_CSMENUCODE, 0, OFFSET_LINUX)

	menu_display(id, menuid, MENU_PAGE_BUY1)
}
...
show_menu_buy2(id)
{
	...
	static menu[250], len, weap, maxloops, wpnname[64]
	len = 0
	...
	{
		get_localized_weapon_name(id, ArrayGetCell(g_secondary_weaponids, weap), wpnname, charsmax(wpnname))
		len += formatex(menu[len], charsmax(menu) - len, "^n\r%d.\w %s", weap+1, wpnname)
	}
...
public show_menu_zclass(id)
{
	...
	// Title
	formatex(menu, charsmax(menu), "\y%s \w%L\r", szTag[MenuTag], id, "MENU_ZCLASS_TITLE")
	...
	for (class = 0; class < g_zclass_i; class++)
	{
		...
		if (class == g_zombieclassnext[id])
			// Seçili olan sınıfın görünümü (Tamamı soluk/gri)
			formatex(menu, charsmax(menu), "\d%s %s %s", szTag[KisaTag], buffer, buffer2)
		else
			// Diğer sınıfların görünümü (Tag gri, İsim beyaz, Özellik sarı)
			formatex(menu, charsmax(menu), "\d%s \w%s \y%s", szTag[KisaTag], buffer, buffer2)
...
// Buy Menu 1
public menu_buy1(id, menuid, item)
{
	// Player disconnected?
	if (!is_user_connected(id))
	{
	...
	// Sayfayi hatirla (ileri/geri/cikis - hepsinde calisir)
	static menudummy
	player_menu_info(id, menudummy, menudummy, MENU_PAGE_BUY1)

	// Menu kapatildi (0 / Cikis)
	if (item == MENU_EXIT)
	{
		menu_destroy(menuid)
		return PLUGIN_HANDLED
	...
	// Silahi zaten aldiysa (baska bir yoldan menu tekrar acildiysa) burada da yakala
	if (!g_canbuy[id])
	{
		zp_colored_print(id, "^x04[Zp]^x01 %L", id, "MENU_BUY1_ALREADY")
		menu_destroy(menuid)
	...
	// Secilen ogenin verisini al (silah index'i ya da -1 = Auto Select)
	static buffer[4], dummy, selection
	menu_item_getinfo(menuid, item, dummy, buffer, charsmax(buffer), _, _, dummy)
	selection = buffer[0]
...
buy_extra_item(id, itemid, ignorecost = 0)
{
	...
	{
		// [DUZELTME] Genel "CMD_NOT" yerine, zombiyken insan esyasi (veya tam tersi) almaya
		// calisildiginda acik ve anlasilir bir uyari veriliyor.
		if (g_zombie[id] && !g_nemesis[id] && !g_assassin[id] && (team & ZP_TEAM_HUMAN))
			zp_colored_print(id, "^x04[ZP]^x01 ^x03Zombiyken bu esyaya erisemezsiniz!")
		else
			zp_colored_print(id, "^x04[Zp]^x01 %L", id, "CMD_NOT")
		return;
	}
...
// Also always hide it for HE Grenade / Flashbang: on this server these are
// reskinned as "Ates Bombasi" / "Buz Bombasi" and the native pickup box
// can't show the custom name (client-side, not scriptable), so we hide it
// entirely and rely on the plugin's own chat/HUD message for the real name.
...
make_a_zombie(mode, id)
{
	...
	&& iPlayersnum-(get_pcvar_num(cvar_plaguesurvnum)+get_pcvar_num(cvar_plaguenemnum)+floatround((iPlayersnum-(get_pcvar_num(cvar_plaguenemnum)+get_pcvar_num(cvar_plaguesurvnum)))*get_pcvar_float(cvar_plagueratio), floatround_ceil)) >= 1 && iPlayersnum >= get_pcvar_num(cvar_plagueminplayers)) || mode == MODE_PLAGUE)
	{
		...
		static iSurvivors, iMaxSurvivors, iSafety
		iMaxSurvivors = get_pcvar_num(cvar_plaguesurvnum)
		// Oyuncu sayisindan fazla survivor/nemesis secilemez; en az 1 kisi
		// zombi/normal rol icin bos birakilmali (sonsuz donguyu engeller)
		if (iMaxSurvivors > iPlayersnum - 1) iMaxSurvivors = iPlayersnum - 1
		if (iMaxSurvivors < 0) iMaxSurvivors = 0
		iSurvivors = 0
		iSafety = 0
		
		while (iSurvivors < iMaxSurvivors && iSafety < 200)
		{
			iSafety++
			// Choose random guy
			id = fnGetRandomAlive(random_num(1, iPlayersnum))
		...
		// Survivor'lardan sonra kalan oyunculardan fazla nemesis secilemez;
		// yine en az 1 kisi normal/zombi rolu icin bos birakilmali
		if (iMaxNemesis > iPlayersnum - iSurvivors - 1) iMaxNemesis = iPlayersnum - iSurvivors - 1
		if (iMaxNemesis < 0) iMaxNemesis = 0
		iNemesis = 0
		iSafety = 0
		
		while (iNemesis < iMaxNemesis && iSafety < 200)
		{
			iSafety++
			// Choose random guy
			id = fnGetRandomAlive(random_num(1, iPlayersnum))
	...
	else if ((mode == MODE_NONE && (!get_pcvar_num(cvar_preventconsecutive) || g_lastmode != MODE_ARMAGEDDON) && random_num(1, get_pcvar_num(cvar_armagechance)) == get_pcvar_num(cvar_armage) && iPlayersnum >= get_pcvar_num(cvar_armageminplayers)) || mode == MODE_ARMAGEDDON)
	{
		// Armageddon Mode
		g_armageround = true
	...
	else if ((mode == MODE_NONE && (!get_pcvar_num(cvar_preventconsecutive) || g_lastmode != MODE_APOCALYPSE) && random_num(1, get_pcvar_num(cvar_apocchance)) == get_pcvar_num(cvar_apoc) && iPlayersnum >= get_pcvar_num(cvar_apocminplayers)) || mode == MODE_APOCALYPSE)
	{
		// Apocalypse Mode
		g_apocround = true
	...
	else if ((mode == MODE_NONE && (!get_pcvar_num(cvar_preventconsecutive) || g_lastmode != MODE_NIGHTMARE) && random_num(1, get_pcvar_num(cvar_nightchance)) == get_pcvar_num(cvar_night) && iPlayersnum >= get_pcvar_num(cvar_nightminplayers)) || mode == MODE_NIGHTMARE)
	{
		// Nightmare mode
		g_nightround = true
			...
			case SECTION_SOUNDS:
			{
				...
				else if (equal(key, "ROUND NEMESIS"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_nemesis_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
				else if (equal(key, "ROUND ASSASSIN"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_assassin_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
				else if (equal(key, "ROUND SURVIVOR"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_survivor_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
				else if (equal(key, "ROUND SNIPER"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_sniper_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
				else if (equal(key, "ROUND SWARM"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_swarm_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
				else if (equal(key, "ROUND MULTI"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_multi_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
				else if (equal(key, "ROUND PLAGUE"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_plague_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
				else if (equal(key, "ROUND ARMAGEDDON"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_armageddon_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
				else if (equal(key, "ROUND APOCALYPSE"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_apocalypse_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
				else if (equal(key, "ROUND NIGHTMARE"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						ArrayPushCell(sound_nightmare_ismp3, equal(key[strlen(key)-4], ".mp3") ? 1 : 0)
					}
				}
			...
			case SECTION_BUY_MENU_WEAPONS:
			{
				if (equal(key, "PRIMARY"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						// BUGFIX: cs_weapon_name_to_id() silently returns 0 (CSW_NONE) when a
						// config weapon name doesn't exactly match WEAPONENTNAMES[]. Adding such
						// an entry to the array gives the item a bogus id, so it shows up in the
						// buy menu but can never actually be given proper ammo -> looks like
						// "can't buy this weapon". Validate and log instead of silently breaking it.
						new wid = cs_weapon_name_to_id(key)
						if (wid <= 0)
						{
							log_amx("[ZP-UYARI] BUY MENU PRIMARY: ^"%s^" gecerli bir silah adiyla eslesmedi (config.ini), menuye eklenmedi.", key)
							continue;
						}
						
						// Add to weapons array
						ArrayPushString(g_primary_items, key)
						ArrayPushCell(g_primary_weaponids, wid)
					}
				}
				else if (equal(key, "SECONDARY"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						// BUGFIX: see PRIMARY parsing above - skip and log unmatched weapon names
						// instead of silently adding a broken (id 0) menu entry.
						new wid2 = cs_weapon_name_to_id(key)
						if (wid2 <= 0)
						{
							log_amx("[ZP-UYARI] BUY MENU SECONDARY: ^"%s^" gecerli bir silah adiyla eslesmedi (config.ini), menuye eklenmedi.", key)
							continue;
						}
						
						// Add to weapons array
						ArrayPushString(g_secondary_items, key)
						ArrayPushCell(g_secondary_weaponids, wid2)
					}
				}
...
public ambience_sound_effects(taskid)
{
	...
	// [FIX] Default values used when the selected mode's ambience array is
	// empty (e.g. "... ENABLE" is on in ambience.ini but the matching
	// "... SOUNDS" / "... DURATIONS" lines are missing or malformed).
	// Without this, ArraySize() returns 0, random_num(0, -1) hands back an
	// out-of-range index, and ArrayGetString crashes with "Invalid index 0
	// (count: 0)". Now we just skip playback for this cycle and retry later.
	sound[0] = 0
	duration = 2
	
	if (g_nemround) // Nemesis Mode
	{
		if (ArraySize(sound_ambience2))
		{
	...
	// Play it on clients (only if a sound was actually found)
	if (sound[0] != 0)
		PlaySound(sound)
	
	// Set the task for when the sound is done playing
	// (falls back to a short 2s retry if the array was empty this cycle)
...
zp_colored_print(target, const message[], any:...)
{
	...
	// [FIX] AMXX's built-in "%L" handling needs one extra argument slot beyond
	// (target, message, langid, key) when %L is the very last thing in the
	// message. Without it, vformat throws "Run time error 25: parameter error"
	// (String formatted incorrectly). Detect that exact shape once and resolve
	// the translation manually via LookupLangKey instead of relying on the
	// crashing recursive "%L" path. Any other call shape (extra trailing args,
	// or %L not at the very end) is left completely untouched below.
	new bool:vulnerableL = (argscount == 4 && strlen(message) >= 2 && equal(message[strlen(message)-2], "%L"))
	new szKeyOnly[64]
	if (vulnerableL)
		vformat(szKeyOnly, charsmax(szKeyOnly), "%s", 4)
	...
	if (!target)
	{
		...
		for (player = 1; player <= g_maxplayers; player++)
		{
			...
			if (vulnerableL)
			{
				new szTrans[256]
				new langId = getarg(2)
				LookupLangKey(szTrans, charsmax(szTrans), szKeyOnly, langId)
				copy(buffer, charsmax(buffer), message)
				buffer[strlen(message)-2] = 0
				add(buffer, charsmax(buffer), szTrans)
			}
			else
			{
...
{
	// Model cleared (costume/aksesuar deactivated): remove the overlay entity
	// and restore the real player mesh to normal rendering instead of leaving
	// it invisible with a stale/garbage overlay stuck on the player.
	if (!g_playermodel[id][0])
	{
